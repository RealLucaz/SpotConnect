...
public class SetupScreen extends Screen {
    ...
    public void extractBackground(@NotNull GuiGraphicsExtractor g, int mouseX, int mouseY, float partial) {
        ...
        g.centeredText(font, "SpotConnect", cx, y, Theme.GREEN);
        g.fill(cx - 34, y + 13, cx + 34, y + 14, Theme.GREEN_DIM);
        y += 30;

        g.centeredText(font, "Connect your Spotify account", cx, y, Theme.TEXT);
        y += 18;

        g.centeredText(font, "Minecraft becomes the interface.", cx, y, Theme.TEXT_MUTED);
        y += 12;
        g.centeredText(font, "Playback runs in a hidden browser window,", cx, y,
                Theme.TEXT_MUTED);
        y += 12;
        g.centeredText(font, "so the Spotify app never needs to be open.", cx, y,
                Theme.TEXT_MUTED);
    }
    ...
    public void extractRenderState(@NotNull GuiGraphicsExtractor g, int mouseX, int mouseY, float partial) {
        ...
        if (!SpotifyConfig.SUPPORTED) {
            g.centeredText(font, "SpotConnect only works on Windows.",
                    cx, contentTop() + 70, Theme.TEXT_ERROR);
            g.centeredText(font, "It drives a dedicated Chrome window using Windows-only",
                    cx, contentTop() + 84, Theme.TEXT_FAINT);
            g.centeredText(font, "APIs. Nothing has been changed on your system.",
                    cx, contentTop() + 94, Theme.TEXT_FAINT);
        } else {
            g.centeredText(font, SpotifyConfig.hasClientId()
        ...
        if (status != null && !status.isEmpty()) {
            ...
            g.centeredText(font, UiText.fit(shown, width - 20), cx, statusY, color);
        }
    }
